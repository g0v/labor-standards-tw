...
describe('工作時間、休息、休假', () => {
  ...
  describe('工作時間', () => {
    it('正常工時情況下，一勞工於一天內工作十個小時，其中八個小時是正常工時，兩個小時為加班', () => {
      var result = std.validateWorkingHours(std.DAILY_WORKING_HOURS, std.REGULAR_WORKING_HOURS, 10);
    ...
    it('正常工時情況下，一勞工於一天內工作十二個小時，其中八個小時是正常工時，四個小時為加班', () => {
      var result = std.validateWorkingHours(std.DAILY_WORKING_HOURS, std.REGULAR_WORKING_HOURS, 12);
      expect(result.status).eq(std.LEGAL);
      expect(result.value.regular).eq(8);
    ...
    it('正常工時情況下，一勞工於一天內工作十五個小時，其中八個小時是正常工時，七個小時為加班並且違反勞基法 32 條', () => {
      var result = std.validateWorkingHours(std.DAILY_WORKING_HOURS, std.REGULAR_WORKING_HOURS, 15);
    ...
    it('正常工時情況下，勞工於一個月內的連續三週（十五天）每天工作十二個小時，此違反勞基法 32 條一個月不能超過 46 小時', () => {
      var hours = Array.apply(null, Array(15)).map(Number.prototype.valueOf, 12);
      ...
      var total = hours
                    ...
                    .reduce((a, b) => a + b);
      var result = std.validateWorkingHours(std.MONTHLY_WORKING_HOURS, std.REGULAR_WORKING_HOURS, total);
      expect(result.status).eq(std.ILLEGAL);
